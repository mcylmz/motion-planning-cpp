 ...
 * RRT* Demo
 *
 * Demonstrates the RRT* (Optimal RRT) algorithm in a continuous 2D
 * environment with geometric obstacles. RRT* extends RRT with cost-aware
 * parent selection and tree rewiring to converge toward the optimal path.
 *
 * Controls:
...
int main() {
    ...
    vizConfig.title = "RRT* - Optimal Rapidly-exploring Random Tree";
    ...
    // RRT* config
    RRTStar::Config rrtStarConfig;
    rrtStarConfig.maxIterations = 5000;
    rrtStarConfig.stepSize = 15.0;
    rrtStarConfig.goalThreshold = 20.0;
    rrtStarConfig.goalBias = 0.05;
    rrtStarConfig.gamma = 50.0;
    rrtStarConfig.continueAfterGoal = true;
    rrtStarConfig.seed = 42;

    RRTStar rrtStar(rrtStarConfig);
    ...
    std::cout << std::endl;
    std::cout << "Continue after goal: " << (rrtStarConfig.continueAfterGoal ? "ON" : "OFF") << std::endl;
    std::cout << std::endl;
    ...
    viz.setKeyCallback([&](sf::Keyboard::Key key) {
        ...
        switch (key) {
            ...
            case sf::Keyboard::Key::T:
                rrtStarConfig.continueAfterGoal = !rrtStarConfig.continueAfterGoal;
                rrtStar.setConfig(rrtStarConfig);
                std::cout << "Continue after goal: "
                          << (rrtStarConfig.continueAfterGoal ? "ON" : "OFF") << std::endl;
                break;
            ...
            case sf::Keyboard::Key::Equal: // + key
                if (mode == Mode::DrawRect) {
                    ...
                    rrtStarConfig.stepSize += 2.0;
                    rrtStar.setConfig(rrtStarConfig);
                    std::cout << "Step size: " << rrtStarConfig.stepSize << std::endl;
                }
                break;
            ...
            case sf::Keyboard::Key::Hyphen: // - key
                if (mode == Mode::DrawRect) {
                    ...
                    rrtStarConfig.stepSize = std::max(2.0, rrtStarConfig.stepSize - 2.0);
                    rrtStar.setConfig(rrtStarConfig);
                    std::cout << "Step size: " << rrtStarConfig.stepSize << std::endl;
                }
                break;
            ...
            case sf::Keyboard::Key::Space: {
                ...
                rrtStarConfig.seed = currentSeed++;
                rrtStar.setConfig(rrtStarConfig);
                ...
                auto vizCallback = [&](const TreeEdge& edge) {
                    ...
                    // Draw all tree edges (rebuilt by algorithm before callback)
                    for (const auto& [parentIdx, childIdx] : rrtStar.getEdges()) {
                        viz.drawLine(rrtStar.getNodes()[parentIdx],
                                    rrtStar.getNodes()[childIdx],
                ...
                resultPath = rrtStar.plan(env, start, goal, vizCallback);
                stats = rrtStar.getStats();
                hasResult = true;
                isRunning = false;
                ...
                if (resultPath.found) {
                    std::cout << "Path found! Length: " << std::fixed
                              ...
                              << ", Nodes: " << rrtStar.getNodes().size()
                              << ", Time: " << std::setprecision(2)
                              << stats.searchTime << "ms" << std::endl;
                } else {
                    std::cout << "No path found after " << rrtStarConfig.maxIterations
                              << " iterations (" << rrtStar.getNodes().size()
                              << " nodes)" << std::endl;
                }
                break;
            }
