 ...
 * RRT-Connect Demo
 *
 * Demonstrates the RRT-Connect algorithm in a continuous 2D environment
 * with geometric obstacles. RRT-Connect grows two trees simultaneously
 * from start and goal, using extend/connect to rapidly find paths.
 *
 * Controls:
 *   Space - Run/Re-run RRT-Connect
 *   R     - Reset trees (keep obstacles)
...
int main() {
    ...
    vizConfig.title = "RRT-Connect - Bidirectional Rapidly-exploring Random Trees";
    ...
    // RRT-Connect config
    RRTConnect::Config rrtConnectConfig;
    rrtConnectConfig.maxIterations = 5000;
    rrtConnectConfig.stepSize = 15.0;
    rrtConnectConfig.connectThreshold = 1.0;
    rrtConnectConfig.seed = 42;

    RRTConnect rrtConnect(rrtConnectConfig);
    ...
    std::cout << "=== RRT-Connect Demo ===" << std::endl;
    std::cout << "Controls:" << std::endl;
    std::cout << "  Space       - Run RRT-Connect" << std::endl;
    std::cout << "  R           - Reset trees" << std::endl;
        ...
        switch (key) {
            ...
            case sf::Keyboard::Key::Equal: // + key
                if (mode == Mode::DrawRect) {
                    ...
                    rrtConnectConfig.stepSize += 2.0;
                    rrtConnect.setConfig(rrtConnectConfig);
                    std::cout << "Step size: " << rrtConnectConfig.stepSize << std::endl;
                }
                break;
            ...
            case sf::Keyboard::Key::Hyphen: // - key
                if (mode == Mode::DrawRect) {
                    ...
                    rrtConnectConfig.stepSize = std::max(2.0, rrtConnectConfig.stepSize - 2.0);
                    rrtConnect.setConfig(rrtConnectConfig);
                    std::cout << "Step size: " << rrtConnectConfig.stepSize << std::endl;
                }
                break;
            ...
            case sf::Keyboard::Key::Space: {
                ...
                rrtConnectConfig.seed = currentSeed++;
                rrtConnect.setConfig(rrtConnectConfig);
                ...
                auto vizCallback = [&](const TreeEdge& edge) {
                    ...
                    // Draw start tree edges (light blue)
                    for (const auto& [parentIdx, childIdx] : rrtConnect.getEdgesStart()) {
                        viz.drawLine(rrtConnect.getNodesStart()[parentIdx],
                                    rrtConnect.getNodesStart()[childIdx],
                                    colors::TREE_EDGE, 1.0f);
                    }

                    // Draw goal tree edges (light coral)
                    for (const auto& [parentIdx, childIdx] : rrtConnect.getEdgesGoal()) {
                        viz.drawLine(rrtConnect.getNodesGoal()[parentIdx],
                                    rrtConnect.getNodesGoal()[childIdx],
                                    colors::TREE_B, 1.0f);
                    }

                    // Highlight newest edge in green
                ...
                resultPath = rrtConnect.plan(env, start, goal, vizCallback);
                stats = rrtConnect.getStats();
                hasResult = true;
                isRunning = false;
                ...
                if (resultPath.found) {
                    std::cout << "Path found! Length: " << std::fixed
                              ...
                              << ", Start tree: " << rrtConnect.getNodesStart().size()
                              << " nodes, Goal tree: " << rrtConnect.getNodesGoal().size()
                              << " nodes, Time: " << std::setprecision(2)
                              << stats.searchTime << "ms" << std::endl;
                } else {
                    std::cout << "No path found after " << rrtConnectConfig.maxIterations
                              << " iterations (Start tree: " << rrtConnect.getNodesStart().size()
                              << " nodes, Goal tree: " << rrtConnect.getNodesGoal().size()
                              << " nodes)" << std::endl;
                }
                break;
